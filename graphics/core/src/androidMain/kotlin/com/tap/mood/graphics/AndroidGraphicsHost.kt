...
fun AndroidGraphicsContent(
    ...
    val graphicsHost = remember { AtomicReference<AndroidGraphicsHost?>() }

    DisposableEffect(controller, graphicsHost) {
        val frameSink: (Frame) -> Unit = { frame -> graphicsHost.get()?.submit(frame) }
        controller.setFrameSink(frameSink)
        onDispose {
            controller.clearFrameSink(frameSink)
            graphicsHost.set(null)
        }
    }
    ...
    AndroidView(
        ...
        onRelease = { view ->
            ...
            graphicsHost.compareAndSet(view, null)
            view.close()
        },
        modifier = modifier,
    )
...
private class AndroidGraphicsHost(
    context: Context,
