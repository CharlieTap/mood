...
internal class AndroidWebGpuDriver private constructor(
    ...
    private val shaderModule: GPUShaderModule,
            ...
            bindGroup =
                device.createBindGroup(
                    GPUBindGroupDescriptor(
                        ...
                        entries =
                            arrayOf(
                                GPUBindGroupEntry(binding = 0, textureView = frameView),
    ...
    @Synchronized
    override fun updateInterpolation(smooth: Boolean) {
        optionsUpload.clear()
        optionsUpload.putInt(if (smooth) 1 else 0)
    ...
    override fun draw(viewport: Viewport): Boolean =
        runCatching { renderFrame(viewport) }.getOrElse { error ->
            Log.e(TAG, "Android WebGPU frontend failed while rendering", error)
            false
        }
    ...
    private fun renderFrame(viewport: Viewport): Boolean {
        ...
        try {
            ...
            pass.setBindGroup(0, requireNotNull(bindGroup))
            pass.draw(3)
            pass.end()
            val commands = encoder.finish()
            val frameResources =
                SubmittedFrameResources(commands, pass, encoder, outputView, outputTexture)
    ...
    internal companion object {
        ...
        private const val OPTIONS_BYTE_COUNT = 16
            ...
            val module =
                device.createShaderModule(
                    GPUShaderModuleDescriptor(
                        label = "Mood palette shader",
                        shaderSourceWGSL = GPUShaderSourceWGSL(PALETTE_WEBGPU_SHADER),
                    ),
                )
            val pipeline =
                device.createRenderPipeline(
                    GPURenderPipelineDescriptor(
                        ...
                        vertex = GPUVertexState(module, "vertexMain"),
